...
import fossilslegacy.server.block.entity.AnalyzerBlockEntity;
import fossilslegacy.server.menu.slot.ResultSlot;
...
public class AnalyzerMenu extends AbstractContainerMenu {
	...
	public AnalyzerMenu(int windowId, Inventory inventory, AnalyzerBlockEntity analyzerBlockEntity) {
		...
		this.addSlot(new ResultSlot(inventory.player, analyzerBlockEntity, 9, 102, 21));
		for (int column = 0; column < 3; ++column) {
			this.addSlot(new ResultSlot(inventory.player, analyzerBlockEntity, column + 10, 98 + column * 18, 53));
		}
