...
public class ArchaeologyRecipeBuilder implements RecipeBuilder {
	...
	private ArchaeologyRecipeBuilder(ItemLike itemLike, Ingredient ingredient, int time) {
		this.result = itemLike.asItem();
		this.ingredient = ingredient;
		this.time = time;
	}

	public static <T extends AbstractCookingRecipe> ArchaeologyRecipeBuilder recipe(Item ingredient, ItemLike itemLike, int time) {
		return new ArchaeologyRecipeBuilder(itemLike, Ingredient.of(ingredient), time);
	}
	...
	public void save(RecipeOutput recipeOutput, ResourceLocation recipeId) {
		...
		recipeOutput.accept(recipeId, new ArchaeologyRecipe(this.ingredient, new ItemStack(this.result), this.time), builder.build(recipeId.withPrefix("recipes/misc/")));
	}
