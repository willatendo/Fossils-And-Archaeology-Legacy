		...
		ModCallbacks.ENTITY_STRUCK_BY_LIGHTING.register((entity, lightningBolt) -> {
			if (lightningBolt instanceof AncientLightningBolt ancientLightningBolt) {
				if (entity instanceof Pig pig) {
					...
					TamedZombifiedPiglin zombifiedPigman = FossilsLegacyEntityTypes.TAMED_ZOMBIFIED_PIGLIN.get().create(level);
					zombifiedPigman.tame(((Player) ancientLightningBolt.getOwner()));
					zombifiedPigman.sendMessageToPlayer(TamedZombifiedPiglin.TamedZombifiedPiglinSpeaker.SUMMON, ((Player) ancientLightningBolt.getOwner()));
