...
public class FABlockModelGenerator extends SimpleBlockModelGenerator {
    ...
    public void run() {
        ...
        this.block(MultiVariantGenerator.multiVariant(FABlocks.AMBER_BLOCK.get(), Variant.variant().with(VariantProperties.MODEL, ModelTemplates.CUBE_ALL.extend().renderType("translucent").build().create(FABlocks.AMBER_BLOCK.get(), new TextureMapping().put(TextureSlot.ALL, this.modLocation("block/amber_block")), this.modelOutput))));
