...
public record DrawEntityLootRecipe(Item spawnEgg, List<List<Drop>> drops) implements DrawLootRecipe {
    public static DrawEntityLootRecipe simple(Item spawnEgg, List<Drop>... drops) {
        return new DrawEntityLootRecipe(spawnEgg, Arrays.asList(drops));
    }
    ...
    public void draw(Level level, SlotPlacer slotPlacer, SpriteDrawer spriteDrawer) {
        ...
        slotPlacer.place(leftPos + 1, 35, new ItemStack(this.spawnEgg));
    ...
    public DrawLootRecipeTypes type() {
        return DrawLootRecipeTypes.ENTITY_LOOT;
    }
