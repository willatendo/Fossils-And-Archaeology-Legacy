...
public class PregnantMammoth extends Mammoth implements DinopediaInformation, PregnantAnimal<Mammoth> {
    private static final EntityDataAccessor<Holder<CoatType>> OFFSPRING_COAT_TYPE = SynchedEntityData.defineId(PregnantMammoth.class, FAEntityDataSerializers.COAT_TYPES.get());
    ...
    public Mammoth getBaseEntity(Level level) {
        return FAEntityTypes.MAMMOTH.get().create(level);
    }
}
