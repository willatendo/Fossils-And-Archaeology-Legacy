...
public class PregnantFox extends Fox implements DinopediaInformation, PregnantAnimal<Fox> {
    private static final EntityDataAccessor<Integer> PREGNANCY_TIME = SynchedEntityData.defineId(PregnantFox.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Holder<PregnancyType>> PREGNANCY_TYPE = SynchedEntityData.defineId(PregnantFox.class, FAEntityDataSerializers.PREGNANCY_TYPES.get());
    private static final EntityDataAccessor<Holder<ModelGene>> OFFSPRING_MODEL_TYPE = SynchedEntityData.defineId(PregnantFox.class, FAEntityDataSerializers.MODEL_TYPES.get());
    private static final EntityDataAccessor<Holder<PatternGene>> OFFSPRING_SKIN = SynchedEntityData.defineId(PregnantFox.class, FAEntityDataSerializers.PATTERN.get());
    private static final EntityDataAccessor<Holder<PatternGene>> OFFSPRING_PATTERN = SynchedEntityData.defineId(PregnantFox.class, FAEntityDataSerializers.PATTERN.get());

    public PregnantFox(EntityType<? extends Fox> entityType, Level level) {
        super(entityType, level);
    }

    @Override
    public ItemStack getPickResult() {
        return Items.FOX_SPAWN_EGG.getDefaultInstance();
    }
    ...
    @Override
    public Fox getBaseEntity(Level level) {
        return EntityType.FOX.create(level, EntitySpawnReason.BREEDING);
    }
}
