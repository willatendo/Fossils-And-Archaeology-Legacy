...
public class SmilodonBegGoal extends Goal {
    ...
    public boolean canUse() {
        this.player = this.level instanceof ServerLevel serverLevel ? serverLevel.getNearestPlayer(this.begTargeting, this.smilodon) : null;
    ...
    private boolean playerHoldingInteresting(Player player) {
        for (InteractionHand interactionHand : InteractionHand.values()) {
            ...
            if (!this.smilodon.isFood(itemStack)) continue;
            return true;
        }
        return false;
    }
