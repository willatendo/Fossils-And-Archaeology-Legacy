...
public class FossilsLegacyFossilVariants {
    ...
    public static final SimpleHolder<FossilVariant> PTERANODON = legacy("pteranodon", 8, 0.125F, 0.5F, 0.5F, fossil -> FossilScaleFactor.create(0.8F + (0.2F * (float) fossil.getSize())));
    public static final SimpleHolder<FossilVariant> TRICERATOPS = legacy("triceratops", 12, 0.15F, 0.5F, 0.5F, fossil -> FossilScaleFactor.create(1.5F + (0.3F * (float) fossil.getSize())));
    ...
    public static SimpleHolder<FossilVariant> withLegacy(String id, int maxSize, float shadowSize, float boundingBoxWidth, float boundingBoxHeight, Function<Fossil, FossilScaleFactor> fossilScaleFactor) {
        return FossilsLegacyFossilVariants.register(id, maxSize, FossilsLegacyUtils.resource("textures/entities/animals/" + id + "/" + id + "_skeleton.png"), FossilsLegacyUtils.resource("textures/entities/animals/" + id + "/legacy/" + id + "_skeleton.png"), shadowSize, boundingBoxWidth, boundingBoxHeight, fossilScaleFactor);
    }
    ...
    public static SimpleHolder<FossilVariant> register(String id, int maxSize, ResourceLocation textureLocation, ResourceLocation legacyTextureLocation, float shadowSize, float boundingBoxWidth, float boundingBoxHeight, Function<Fossil, FossilScaleFactor> fossilScaleFactor) {
        return FOSSIL_VARIANTS.register(id, () -> new FossilVariant(maxSize, textureLocation, legacyTextureLocation, shadowSize, boundingBoxWidth, boundingBoxHeight, fossilScaleFactor));
    }
